...
async function tasksRoutes(fastify: FastifyInstance) {
    ...
    fastify.get<{ Params: { guid: string } }>(
        "/:guid",
        { schema: GetTaskSchema, preHandler: authMiddleware },
        TaskController.GetTask
    );
    fastify.put<{ Params: { guid: string } }>(
        "/:guid",
        { schema: UpdateTaskSchema, preHandler: authMiddleware },
        TaskController.UpdateTask
    );
    fastify.patch<{ Params: { guid: string } }>(
        "/:guid",
        { schema: PatchTaskSchema, preHandler: authMiddleware },
        TaskController.PatchTask
    );
    fastify.delete<{ Params: { guid: string } }>(
        "/:guid",
        { schema: DeleteTaskSchema, preHandler: authMiddleware },
