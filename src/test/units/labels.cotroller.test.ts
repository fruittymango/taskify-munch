...
describe("Manage labels", () => {
    ...
    test("should get labels - labels not do exist", async () => {
        const mockRequest: Partial<any> = { body: { ...testLabel } };
    ...
    test("should not add label - label do exist (duplicate title)", async () => {
        ...
        try {
            await LabelsController.AddLabel(
                mockRequest as any,
                mockReply as any
            );
        } catch (error) {
            if (error instanceof Error) {
                expect(error.message).toBe("Failed to add label.");
    ...
    test("should add label - labels do exist", async () => {
        ...
        const mockRequest: Partial<any> = { body: { test, ...newLabel } };
        const mockReply: Partial<any> = {
            status: jest.fn().mockReturnThis(),
            statusCode: jest.fn(),
            send: jest.fn(),
    ...
    test("should add bulk labels - labels do not exist", async () => {
        const result = await addBulkLabels([
            { title: humanId() },
            { title: humanId() },
        ]);
