...
describe("Manage tasks", () => {
    ...
    test("should update tasks - task does exist", async () => {
        const mockGetTasksRequest: Partial<any> = {
            body: {},
            user: { ...testingUser },
            query: { projectGuid: testingProject.guid },
        };
        ...
        await TaskController.GetTasks(
            mockGetTasksRequest as any,
            mockGetTasksReply as any
        );
    ...
    test("should not patch tasks status - task does not exist", async () => {
        ...
        try {
            await TaskController.PatchTask(
                mockRequest as any,
                mockReply as any
            );
        } catch (error) {
    ...
    test("should delete tasks status - task does exist", async () => {
        ...
        const mockRequest: Partial<any> = {
            ...
            user: { ...testingUser },
            params: { guid: task.guid },
            query: { projectGuid: testingUser.guid },
        };
        const mockReply: Partial<any> = {
            status: jest.fn().mockReturnThis(),
            statusCode: jest.fn(),
