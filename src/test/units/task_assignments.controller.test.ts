...
const generateNewProject = () => {
    return { title: humanId(), description: humanId(), guid: uuidv4() };
};
const testingProject = { ...generateNewProject(), userId: 1, id: 1 };
const testingProjects = [
    testingProject,
    { userId: 2, ...generateNewProject() },
    { userId: 3, ...generateNewProject() },
];
...
describe("Manage tasks assignments", () => {
    ...
    test("should not add one task assignments - task does not exist", async () => {
        ...
        try {
            await TaskAssignmentsController.AddTaskAssignment(
                mockRequest as any,
                mockReply as any
            );
        } catch (error) {
    ...
    test("should not add one task assignment - user does not exist", async () => {
        ...
        try {
            ...
            if (error instanceof Error) {
                expect(error.message).toBe("Failed to get user by id.");
                expect(error.name).toBe("DatabaseRelatedError");
            }
        }
    });
    test("should add one task assignments - task does exist", async () => {
        // Get project tasks
        const mockAssignTaskRequest: Partial<any> = {
            body: { userId: 1 },
            user: { ...testingUser },
            params: { guid: testTask.guid },
        };
        ...
        await TaskAssignmentsController.AddTaskAssignment(
            mockAssignTaskRequest as any,
            mockAssignTasksReply as any
        );
    ...
    test("should not remove one task assignments - task does not exist", async () => {
        ...
        try {
            await TaskAssignmentsController.DeleteTaskAssignment(
                mockRequest as any,
                mockReply as any
            );
        } catch (error) {
