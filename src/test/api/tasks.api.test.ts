    ...
    describe("authorised access of resource", () => {
        ...
        describe("getting one task", () => {
            ...
            test("should get one task - task guid does exist", async () => {
                ...
                const tasks = await axios.get(
                    "http://127.0.0.1:5000/tasks?projectGuid=" +
                        projects.data[projectIndex].guid
                );
                ...
                const task = await axios.get(
                    "http://127.0.0.1:5000/tasks/" + tasks.data[taxIndex].guid
                );

                expect(task.data.title).toBe(tasks.data[taxIndex].title);
                expect(task.data.projectId).toBe(
                    tasks.data[taxIndex].projectId
                );
        ...
        describe("updating one task", () => {
            test("should not update one tasks - schema invalid", async () => {
                try {
                    ...
                    await axios.put("http://127.0.0.1:5000/tasks/23ws", {
                        ...editTask,
                    });
                } catch (error: Error | AxiosError | any) {
                    if (error instanceof AxiosError) {
            ...
            test("should not update task - task does not exist", async () => {
                try {
                    ...
                    await axios.put("http://127.0.0.1:5000/tasks/" + uuidv4(), {
                        ...editTaskBody,
                    });
                } catch (error: Error | AxiosError | any) {
                    if (error instanceof AxiosError) {
            ...
            test("should update task - task does exist", async () => {
                ...
                const editTaskResult = await axios.put(
                    "http://127.0.0.1:5000/tasks/" + task.data[0].guid,
                    { ...editTaskBody }
                );
                expect(editTaskResult.status).toBe(200);
                expect(editTaskResult.data.title).toBe(editTaskBody.title);
                expect(editTaskResult.data.description).toBe(
                    editTaskBody.description
                );
            });
        });
        ...
        describe("patching one task", () => {
            test("should patch task status - task does exist", async () => {
                ...
                const editTaskResult = await axios.patch(
                    "http://127.0.0.1:5000/tasks/" + tasks.data[0].guid,
                    { ...pacthTaskBody }
                );
                expect(editTaskResult.status).toBe(200);
                expect(editTaskResult.data.statusId).toBe(
                    pacthTaskBody.statusId
                );
            });
        });
        ...
        describe("deleting one task", () => {
            test("should not delete task - task does not exist", async () => {
                try {
                    await axios.delete(
                        "http://127.0.0.1:5000/tasks/" + uuidv4()
                    );
                } catch (error: Error | AxiosError | any) {
                    if (error instanceof AxiosError) {
                        expect(error.response?.status).toBe(422);
            ...
            test("should delete task - task does exist", async () => {
                ...
                const editTaskResult = await axios.delete(
                    "http://127.0.0.1:5000/tasks/" + tasks.data[0].guid
                );
                expect(editTaskResult.status).toBe(200);
            });
